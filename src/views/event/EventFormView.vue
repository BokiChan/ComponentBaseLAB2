...
function saveEvent() {
    EventService.saveEvent(event.value)
        .then((response) => {
            ...
            store.updateMessage(`You are successsfully add a new event for` + response.data.title)
            setTimeout(() => {
                store.resetMessage()
            }, 3000)
        })
...
hover:border-emerald-500 hover: shadow-lg active: scale-100 focus:outline-none" type="submit">Submit</button>
        </form>
